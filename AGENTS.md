...
4. `docker/test-compose.yml` (generated by `tests/setup-test-env.py`) mounts `docker/tables.sql`/`docker/structure.sql`/`docker/seed-data.sql` directly into the test postgres container, so schema changes are picked up automatically - no copy step to keep in sync.
