...
def test_logs_control_character_handling():
    """Test that control characters in logs are automatically cleaned and don't cause database errors"""
    runner = ScenarioRunner(uri=GMT_DIR, uri_type='folder', filename='tests/data/usage_scenarios/capture_logs_with_control_characters.yml',
                          skip_system_checks=True, dev_cache_build=True, dev_no_sleeps=True,
                          dev_no_metrics=True, dev_no_phase_stats=True, dev_no_save=False)

    # This should not raise any database errors despite control characters in the scenario
    run_id = runner.run()
    ...
    # Verify no problematic control characters remain in stored logs
    logs = logs_result[0]
    container_logs = logs["test-container"]

    # Control characters that should be removed (matches the regex pattern in _sanitize_text)
    # Excludes tab (\x09), newline (\x0A), carriage return (\x0D) which are preserved
    problematic_chars = (
        set(chr(i) for i in range(0, 9)) |      # \x00-\x08: null through backspace
        {'\x0B', '\x0C'} |                       # \x0B (vtab), \x0C (form feed)
        set(chr(i) for i in range(14, 32)) |    # \x0E-\x1F: other control chars
        {'\x7F'}                                 # \x7F: delete character
    )

    for log_entry in container_logs:
        # Check stdout and stderr specifically since these are the fields that get sanitized
        for field in ['stdout', 'stderr']:
            if field in log_entry and log_entry[field] is not None:
                output_text = log_entry[field]
                for char in problematic_chars:
                    assert char not in output_text, f"Control character {repr(char)} should be automatically cleaned from {field}: {repr(output_text)}"
