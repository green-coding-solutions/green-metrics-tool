...
def test_uri_local_dir():
    ...
    ps = subprocess.run(
        ...
        '--filename', filename,
        '--dev-no-system-checks', '--dev-no-sleeps', '--dev-cache-build', '--dev-no-metrics', '--dev-no-phase-stats',
...
def test_runner_with_iterations_and_save_to_database():
    ...
    ps = subprocess.run(
        ['python3', f'{GMT_DIR}/runner.py', '--uri', GMT_DIR,
         ...
         '--iterations', '2',
         '--config-override', f"{os.path.dirname(os.path.realpath(__file__))}/test-config.yml",
         '--dev-no-system-checks', '--dev-cache-build', '--dev-no-sleeps', '--dev-no-metrics', '--dev-no-phase-stats',
         '--skip-optimizations', '--skip-download-dependencies', '--dev-no-container-dependency-collection'],
        check=True,
        stderr=subprocess.PIPE,
        stdout=subprocess.PIPE,
...
def test_file_cleanup():
    subprocess.run(
        ['python3', f'{GMT_DIR}/runner.py', '--uri', GMT_DIR, '--filename', 'tests/data/usage_scenarios/basic_stress.yml',
         ...
         '--dev-no-system-checks', '--dev-no-sleeps', '--dev-cache-build', '--dev-no-save',
         '--skip-optimizations', '--skip-download-dependencies', '--dev-no-container-dependency-collection'],
        check=True,
        stderr=subprocess.PIPE,
        stdout=subprocess.PIPE,
...
def wip_test_verbose_provider_boot():
    ...
    ps = subprocess.run(
        ['python3', f'{GMT_DIR}/runner.py', '--name', run_name, '--uri', GMT_DIR,
         ...
         '--filename', 'tests/data/stress-application/usage_scenario.yml',
         '--dev-no-sleeps', '--dev-cache-build', '--dev-no-metrics', '--dev-no-phase-stats',
         '--skip-optimizations', '--skip-download-dependencies', '--dev-no-container-dependency-collection'],
        check=True,
        stderr=subprocess.PIPE,
        stdout=subprocess.PIPE,
