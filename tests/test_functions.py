...
for phase in TEST_MEASUREMENT_PHASES:
    if ']' not in phase['name'] and not phase['hidden']:
        TEST_MEASUREMENT_RUNTIME_DURATION_NON_HIDDEN += phase['end'] -  phase['start']

TEST_MEASUREMENT_RUNTIME_DURATION_HIDDEN = 0
for phase in TEST_MEASUREMENT_PHASES:
    if ']' not in phase['name'] and phase['hidden']:
        TEST_MEASUREMENT_RUNTIME_DURATION_HIDDEN += phase['end'] -  phase['start']
...
def filter_df_runtime_subphase(df, *, hidden=False, phase_name=None, concat=True):
    df_list = []
    for phase in TEST_MEASUREMENT_PHASES:
        if phase_name:
