...
def test_phase_stats_single_container():
    run_id = Tests.insert_run(Tests.TEST_MEASUREMENT_PHASES)
    df = Tests.import_cpu_utilization_container(run_id)
...
def test_phase_stats_network_io_in_hidden_phase():
    run_id = Tests.insert_run(Tests.TEST_MEASUREMENT_PHASES)
    df_network_io = Tests.import_network_io_procfs(run_id, filename='network_io_procfs_system_in_hidden_phase.log')
