...
def carbondb_add(client_ip, energydatas, source, user_id):
    ...
    query = '''
            INSERT INTO carbondb_energy_data
    ...
    for e in energydatas:
        ...
        fields_to_check = {
            'energy': e['energy'], # is expected to be in J
        ...
        energy_kwh = float(e['energy']) * 2.77778e-7 # kWh
        carbon = energy_kwh * carbon_intensity # results in g

        DB().query(query=query, params=(e['type'], e['project'], e['machine'], source, e['tags'], e['time'], e['energy'], carbon, carbon_intensity, latitude, longitude, used_client_up, user_id))

def validate_carbondb_params(param, elements: list):
    for el in elements:
        if not re.fullmatch(r'[A-Za-z0-9_-]+', el):
            raise ValueError(f"Parameter for '{param}' may only contain A-Za-z0-9_- characters and no spaces. Was: {el}")
