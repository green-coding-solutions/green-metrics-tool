...
def freeze_dict(d):
    ...
    if not hasattr(d, '__class__') and isinstance(d, object): # hasattr __class__ separates actual defined classes from builtins like str
        raise RuntimeError(f"freeze_dict received object of type {type(d)} in its config initalization. This is not expected and leads to issues as it cannot be made immutable!")
