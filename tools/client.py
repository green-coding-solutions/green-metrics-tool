...
STATUS_LIST = ['job_no', 'job_start', 'job_error', 'job_end', 'cleanup_start', 'cleanup_end', 'measurement_control_start', 'measurement_control_end', 'measurement_control_error']
...
def set_status(status_code, cur_temp, cooldown_time_after_job, data=None, run_id=None):
    ...
    query = """
        UPDATE machines
        SET status_code=%s, cooldown_time_after_job=%s, cur_temp=%s, base_temperature=%s, jobs_processing=%s, gmt_hash=%s, gmt_timestamp=%s
        WHERE id = %s
    """
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if current_temperature > config_main['machine']['base_temperature_value']:
            print(f"Machine is still to hot: {current_temperature}°. Sleeping for 1 minute")
            cooldown_time += 60
            time.sleep(60)
            continue
        ...
        if validate.is_validation_needed(client_main['time_between_control_workload_validations']):
            set_status('measurement_control_start', current_temperature, cooldown_time)
            validate.run_workload(cwl['name'], cwl['uri'], cwl['filename'], cwl['branch'])
            set_status('measurement_control_end', current_temperature, cooldown_time)
            ...
            try:
                ...
                set_status('measurement_control_error', current_temperature, cooldown_time)
                ...
                do_cleanup(current_temperature, cooldown_time)

        elif job:
            set_status('job_start', current_temperature, cooldown_time, run_id=job._run_id)
            try:
                job.process(docker_prune=True)
                set_status('job_end', current_temperature, cooldown_time, run_id=job._run_id)
            except Exception as exc:
                set_status('job_error', current_temperature, cooldown_time, data=str(exc), run_id=job._run_id)
            ...
            do_cleanup(current_temperature, cooldown_time)
            set_status('job_no', current_temperature, cooldown_time)
