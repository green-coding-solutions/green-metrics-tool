...
def _do_email_job(job_id, run_id):
    ...
    if (config['admin']['notify_admin_for_own_run_ready'] or config['admin']['email'] != email):
...
def handle_job_exception(exce, p_id):
    ...
    client_mail = None
    if p_id:
        [run_name, _, client_mail, _, _, machine] = get_run(p_id)
    ...
    email_helpers.send_error_email(GlobalConfig().config['admin']['email'], error_helpers.format_error(
        'Base exception occurred in jobs.py: ', exce), run_id=p_id, name=run_name, machine=machine)

    # reduced error message to client
    if client_mail and GlobalConfig().config['admin']['email'] != client_mail:
        email_helpers.send_error_email(client_mail, exce, run_id=p_id, name=run_name, machine=machine)
