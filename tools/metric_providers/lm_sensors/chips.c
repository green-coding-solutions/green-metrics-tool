...
static inline double deg_ctof(double cel) { return cel * (9.0F / 5.0F) + 32.0F; }
...
void print_chip_raw(const sensors_chip_name *name) {
    ...
    while ((feature = sensors_get_features(name, &a))) {
        if (!(label = sensors_get_label(name, feature))) {
            fprintf(stderr,
                    "ERROR: Can't get label of feature "
                    "%s!\n",
                    feature->name);
            continue;
        }
        ...
        while ((sub = sensors_get_all_subfeatures(name, feature, &b))) {
            if (sub->flags & SENSORS_MODE_R) {
                if ((err = sensors_get_value(name, sub->number, &val))) {
                    ...
                    if (fahrenheit) val = deg_ctof(val);
                    printf("  %s: %.3f\n", sub->name, val);
                }
            } else {
                printf("(%s)\n", label);
...
int get_label_size(const sensors_chip_name *name) {
    ...
    while ((iter = sensors_get_features(name, &i))) {
        if ((label = sensors_get_label(name, iter)) && strlen(label) > max_size) max_size = strlen(label);
        free(label);
    }
...
static void print_alarms(struct sensor_subfeature_data *alarms, int alarm_count, int leading_spaces) {
    int i, printed;
...
static void print_limits(struct sensor_subfeature_data *limits, int limit_count, struct sensor_subfeature_data *alarms,
                         int alarm_count, int label_size, const char *fmt) {
    int i, slot, skip;
    ...
    for (i = slot = 0; i < limit_count; i++, slot++) {
        ...
        printf(fmt, limits[i].name, limits[i].value, limits[i].unit);

        /* If needed, skip one slot to avoid hyst on first column */
        skip = i + 2 < limit_count && limits[i + 2].name == hyst_str && !(slot & 1);

        if (((slot + skip) & 1) || i == limit_count - 1) {
            printf(")");
            if (alarm_count && !alarms_printed) {
                print_alarms(alarms, alarm_count, (slot & 1) ? 0 : 16);
                alarms_printed = 1;
            }
        }
        slot += skip;
    }
    if (alarm_count && !alarms_printed) print_alarms(alarms, alarm_count, 32);
}
 ...
 */
static void get_sensor_limit_data(const sensors_chip_name *name, const sensors_feature *feature,
                                  const struct sensor_subfeature_list *sfl, struct sensor_subfeature_data *limits,
                                  int *num_limits, struct sensor_subfeature_data *alarms, int *num_alarms) {
    ...
    for (; sfl->subfeature >= 0; sfl++) {
        ...
        if (sf) {
            ...
            if (sfl->exists) {
                get_sensor_limit_data(name, feature, sfl->exists, limits, num_limits, alarms, num_alarms);
            }
        }
    }
}

static const struct sensor_subfeature_list temp_min_sensors[] = {{SENSORS_SUBFEATURE_TEMP_MIN_HYST, NULL, 0, hyst_str},
                                                                 {-1, NULL, 0, NULL}};

static const struct sensor_subfeature_list temp_lcrit_sensors[] = {
    {SENSORS_SUBFEATURE_TEMP_LCRIT_HYST, NULL, 0, hyst_str}, {-1, NULL, 0, NULL}};

static const struct sensor_subfeature_list temp_max_sensors[] = {{SENSORS_SUBFEATURE_TEMP_MAX_HYST, NULL, 0, hyst_str},
                                                                 {-1, NULL, 0, NULL}};

static const struct sensor_subfeature_list temp_crit_sensors[] = {
    {SENSORS_SUBFEATURE_TEMP_CRIT_HYST, NULL, 0, hyst_str}, {-1, NULL, 0, NULL}};

static const struct sensor_subfeature_list temp_emergency_sensors[] = {
    {SENSORS_SUBFEATURE_TEMP_EMERGENCY_HYST, NULL, 0, hyst_str}, {-1, NULL, 0, NULL}};
...
static const struct sensor_subfeature_list temp_sensors[] = {
    ...
    {SENSORS_SUBFEATURE_TEMP_EMERGENCY, temp_emergency_sensors, 0, "emerg"},
...
#define NUM_TEMP_SENSORS                                                                       \
    (ARRAY_SIZE(temp_sensors) + ARRAY_SIZE(temp_max_sensors) + ARRAY_SIZE(temp_crit_sensors) + \
     ARRAY_SIZE(temp_emergency_sensors) - NUM_TEMP_ALARMS - 4)

void print_chip_temp(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    ...
    sf = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_TEMP_FAULT);
    ...
    get_sensor_limit_data(name, feature, temp_sensors, sensors, &sensor_count, alarms, &alarm_count);

    for (i = 0; i < sensor_count; i++) {
        if (fahrenheit) sensors[i].value = deg_ctof(sensors[i].value);
        sensors[i].unit = degstr;
    }

    print_limits(sensors, sensor_count, alarms, alarm_count, label_size, "%-4s = %+5.1f%s");

    /* print out temperature sensor info */
    sf = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_TEMP_TYPE);
    if (sf) {
...
int chip_temp_raw(const sensors_chip_name *name, const sensors_feature *feature) {
    const sensors_subfeature *sf;
    double val;
...
static const struct sensor_subfeature_list voltage_sensors[] = {{SENSORS_SUBFEATURE_IN_ALARM, NULL, 1, NULL},
...
static void print_chip_in(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    const sensors_subfeature *sf;
    char *label;
    const char *unit;
    ...
    sensor_count = alarm_count = 0;
    get_sensor_limit_data(name, feature, voltage_sensors, sensors, &sensor_count, alarms, &alarm_count);

    print_limits(sensors, sensor_count, alarms, alarm_count, label_size, "%s = %+6.2f V");

    printf("\n");
}

static void print_chip_fan(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    ...
    sfmax = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_FAN_MAX);
    sfdiv = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_FAN_DIV);
    if (sfmin || sfmax || sfdiv) {
        printf("  (");
        if (sfmin) printf("min = %4.0f RPM", get_value(name, sfmin));
        if (sfmax) printf("%smax = %4.0f RPM", sfmin ? ", " : "", get_value(name, sfmax));
        if (sfdiv) printf("%sdiv = %1.0f", (sfmin || sfmax) ? ", " : "", get_value(name, sfdiv));
        printf(")");
    }

    sf = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_FAN_ALARM);
    sfmin = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_FAN_MIN_ALARM);
    sfmax = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_FAN_MAX_ALARM);
    if ((sf && get_value(name, sf)) || (sfmin && get_value(name, sfmin)) || (sfmax && get_value(name, sfmax)))
        printf("  ALARM");

    printf("\n");
}

int chip_fan_raw(const sensors_chip_name *name, const sensors_feature *feature) {
    const sensors_subfeature *sf;
    double val;
...
static void scale_value(double *value, const char **prefixstr) {
    ...
    static struct scale_table prefix_scales[] = {
        {1e-6, "n"}, {1e-3, "u"}, {1, "m"}, {1e3, ""}, {1e6, "k"}, {1e9, "M"}, {0, "G"}, /* no upper bound */
    };
...
static const struct sensor_subfeature_list power_common_sensors[] = {
    {SENSORS_SUBFEATURE_POWER_ALARM, NULL, 1, NULL},        {SENSORS_SUBFEATURE_POWER_MIN_ALARM, NULL, 1, "MIN"},
    {SENSORS_SUBFEATURE_POWER_MAX_ALARM, NULL, 1, "MAX"},   {SENSORS_SUBFEATURE_POWER_LCRIT_ALARM, NULL, 1, "LCRIT"},
    {SENSORS_SUBFEATURE_POWER_CRIT_ALARM, NULL, 1, "CRIT"}, {SENSORS_SUBFEATURE_POWER_CAP_ALARM, NULL, 1, "CAP"},
    {SENSORS_SUBFEATURE_POWER_MAX, NULL, 0, "max"},         {SENSORS_SUBFEATURE_POWER_MIN, NULL, 0, "min"},
    {SENSORS_SUBFEATURE_POWER_LCRIT, NULL, 0, "lcrit"},     {SENSORS_SUBFEATURE_POWER_CRIT, NULL, 0, "crit"},
    {SENSORS_SUBFEATURE_POWER_CAP, NULL, 0, "cap"},         {-1, NULL, 0, NULL}};
...
static const struct sensor_subfeature_list power_inst_sensors[] = {
    ...
    {SENSORS_SUBFEATURE_POWER_AVERAGE_INTERVAL, NULL, 0, "interval"},
    {-1, NULL, 0, NULL}};
...
#define NUM_POWER_SENSORS (ARRAY_SIZE(power_common_sensors) + ARRAY_SIZE(power_inst_sensors) - NUM_POWER_ALARMS - 2)

static void print_chip_power(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    double val;
    const sensors_subfeature *sf;
     ...
     */
    sf = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_POWER_INPUT);
    get_sensor_limit_data(name, feature, sf ? power_inst_sensors : power_avg_sensors, sensors, &sensor_count, alarms,
                          &alarm_count);
    /* Add sensors common to both flavors. */
    get_sensor_limit_data(name, feature, power_common_sensors, sensors, &sensor_count, alarms, &alarm_count);
    if (!sf) sf = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_POWER_AVERAGE);
    ...
    for (i = 0; i < sensor_count; i++) {
        ...
        if (strcmp(sensors[i].name, "interval")) {
            ...
            snprintf(tmpstr, 4, "%sW", unit);  // NOLINT(runtime/printf) We know that this is 4. See two lines up!
            sensors[i].unit = tmpstr;
        } else {
            sensors[i].unit = "s";
        }
    }
    print_limits(sensors, sensor_count, alarms, alarm_count, label_size, "%s = %6.2f %s");

    printf("\n");
}

static void print_chip_energy(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    double val;
    const sensors_subfeature *sf;
    char *label;
    const char *unit;
    ...
    sf = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_ENERGY_INPUT);
    if (sf && get_input_value(name, sf, &val) == 0) {
        scale_value(&val, &unit);
...
static void print_chip_vid(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    char *label;
    const sensors_subfeature *subfeature;
    double vid;

    subfeature = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_VID);
    if (!subfeature) return;

    if ((label = sensors_get_label(name, feature)) && !sensors_get_value(name, subfeature->number, &vid)) {
        print_label(label, label_size);
        printf("%+6.3f V\n", vid);
    }
    free(label);
...
static void print_chip_humidity(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    char *label;
    const sensors_subfeature *subfeature;
    double humidity;

    subfeature = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_HUMIDITY_INPUT);
    if (!subfeature) return;

    if ((label = sensors_get_label(name, feature)) && !sensors_get_value(name, subfeature->number, &humidity)) {
        print_label(label, label_size);
        printf("%6.1f %%RH\n", humidity);
    }
    free(label);
...
static void print_chip_beep_enable(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    char *label;
    const sensors_subfeature *subfeature;
    double beep_enable;

    subfeature = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_BEEP_ENABLE);
    if (!subfeature) return;

    if ((label = sensors_get_label(name, feature)) && !sensors_get_value(name, subfeature->number, &beep_enable)) {
        print_label(label, label_size);
...
static const struct sensor_subfeature_list current_sensors[] = {{SENSORS_SUBFEATURE_CURR_ALARM, NULL, 1, NULL},
...
static void print_chip_curr(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    const sensors_subfeature *sf;
    double val;
    char *label;
    const char *unit;
    ...
    sf = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_CURR_INPUT);
    if (sf && get_input_value(name, sf, &val) == 0) {
        scale_value(&val, &unit);
    ...
    sensor_count = alarm_count = 0;
    get_sensor_limit_data(name, feature, current_sensors, sensors, &sensor_count, alarms, &alarm_count);

    print_limits(sensors, sensor_count, alarms, alarm_count, label_size, "%s = %+6.2f A");

    printf("\n");
}

static void print_chip_intrusion(const sensors_chip_name *name, const sensors_feature *feature, int label_size) {
    char *label;
    const sensors_subfeature *subfeature;
    double alarm;

    subfeature = sensors_get_subfeature(name, feature, SENSORS_SUBFEATURE_INTRUSION_ALARM);
    if (!subfeature) return;

    if ((label = sensors_get_label(name, feature)) && !sensors_get_value(name, subfeature->number, &alarm)) {
        print_label(label, label_size);
        printf("%s\n", alarm ? "ALARM" : "OK");
    }
    free(label);
