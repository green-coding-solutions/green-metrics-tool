...
class PsuEnergyAcSdiaMachineProvider(BaseMetricProvider):
    ...
    def check_system(self, check_command="default", check_error_message=None, check_parallel_provider=True):
        ...
        if not configured_providers['psu_energy_ac_sdia_machine']['CPUChips']:
        ...
        if 'cpu_utilization_mach_system' not in configured_providers and 'cpu_utilization_procfs_system' not in configured_providers and 'cpu_utilization_win32_system' not in configured_providers:
            raise MetricProviderConfigurationError(f"{self._metric_name} provider could not be started.\nPlease activate a CPU Utilization provider (cpu_utilization_mach_system / cpu_utilization_procfs_system / cpu_utilization_win32_system) in the config.yml\n \
    ...
    def _read_metrics(self):
        ...
        if not self._filename:
            if self._folder.joinpath('cpu_utilization_procfs_system.log').exists():
                ...
                raise RuntimeError(f"could not find the cpu_utilization_procfs_system.log, cpu_utilization_mach_system.log or cpu_utilization_win32_system.log file in {self._folder}. \
                    Did you activate the cpu_utilization_mach_system, cpu_utilization_procfs_system or cpu_utilization_win32_system in the config.yml too? \
                    This is required to run PsuEnergyAcSdiaMachineProvider")
