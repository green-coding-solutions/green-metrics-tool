...
class CpuEnergyRaplEmiComponentProvider(BaseMetricProvider):

    def __init__(self, sampling_rate, folder, skip_check=False):
        self._stdout_file = None
        super().__init__(
            metric_name='cpu_energy_rapl_emi_component',
    ...
    def check_system(self, check_command="default", check_error_message=None,
        ...
        return super().check_system(
            ...
            check_error_message=(
                ...
                'and that the Windows Energy Meter Interface (EMI) driver is active. '
                'This provider requires Windows 11 with an Intel or AMD CPU '
                '(inbox RAPL driver) or dedicated hardware energy meters.'
            ),
            check_parallel_provider=False,
        )
