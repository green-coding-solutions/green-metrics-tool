...
CREATE TABLE ci_measurements (
    ...
    city text,
    carbon_intensity_g int CHECK (carbon_intensity_g > 0::double precision),
    carbon_ug bigint,
...
CREATE TABLE carbondb_data_raw (
    ...
    carbon_intensity_g int CHECK (carbon_intensity_g > 0::double precision), -- we need this column nullable as we want to backfill missing data
    latitude DOUBLE PRECISION,
    longitude DOUBLE PRECISION,
    ip_address INET,
...
CREATE TABLE hog_simplified_measurements (
    ...
    carbon_intensity_g DOUBLE PRECISION CHECK (carbon_intensity_g > 0::double precision),
