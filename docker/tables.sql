-- Schema/extensions/tables, re-run on every tests/test_functions.py::reset_db() call (as well as
-- once at first container boot, right after structure.sql creates the database itself - see the
-- comment there). Every statement in this file must stay safely repeatable (IF NOT EXISTS /
-- idempotent), since it targets whatever schema is currently first in search_path - either a
-- pytest-xdist worker's own private schema, or "public" itself for non-parallel test runs.
CREATE SCHEMA IF NOT EXISTS "public";

-- Extensions are registered once per database (not per schema), so pin them to "public"
-- explicitly and make them idempotent: reset_db() re-runs this file into a fresh per-worker
-- schema on every reset, and a bare CREATE EXTENSION would fail on every worker after the first
-- (or every reset after the first, outside of xdist).
...
CREATE TABLE measurement_values (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- although not strictly needed PostgreSQL seems to perform way better with it and can vacuum more efficiently
