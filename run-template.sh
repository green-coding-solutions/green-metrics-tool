...
if [[ "$1" == "website" ]]; then
    ...
    if [[ -n "${3-}" && "${3-}" == '--quick' ]]; then
        python3 "${GMT_ROOT_DIR}/runner.py" --uri ${GMT_ROOT_DIR} --filename 'templates/ai/usage_scenario.yml' --variables __GMT_VAR_PROMPT__="${2}"  --dev-no-sleeps --skip-system-checks --dev-cache-build --dev-no-optimizations --allow-unsafe ${4-} ${5-}
    else
        python3 "${GMT_ROOT_DIR}/runner.py" --uri ${GMT_ROOT_DIR} --filename 'templates/ai/usage_scenario.yml' --variables __GMT_VAR_PROMPT__="${2}" --allow-unsafe ${3-} ${4-}
    fi
else
